...
# 🔐 Practical 3 Recap: Multi-Factor Authentication (MFA) for IAM Users

## ✅ What We Did

- Accessed the **IAM dashboard**
- Selected an IAM user (`testuser`)
- Enabled **MFA (Multi-Factor Authentication)** using a virtual authenticator app (e.g., Google Authenticator)
...
| Benefit                  | Description |
|--------------------------|-------------|
| 👁️‍🗨️ Increased Security      | Stops attackers even if credentials are stolen |
| 🔒 Prevents Unauthorized Access | Blocks access unless both password and TOTP are verified |
| ⚙️ Easy to Set Up         | Configured in minutes using common mobile apps |
...
- ❌ Remove MFA device if no longer needed (via IAM > User > Security Credentials)
- ✅ Retain MFA setup if continuing to use this user securely
